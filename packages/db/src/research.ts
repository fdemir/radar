...
export function createResearch(db: Database) {
  ...
  async function start(userId: string, taskId: string, now = Date.now()) {
    ...
    try {
      await raw.batch([
        raw.prepare("INSERT INTO run (id, task_id, user_id, revision, started) VALUES (?, ?, ?, ?, ?)").bind(id, taskId, userId, current.revision, now),
        raw.prepare("UPDATE task SET next_run_at = ? WHERE id = ? AND (next_run_at IS NULL OR next_run_at <= ?)").bind(next, taskId, now),
      ]);
    } catch (error) {
      const detail = error instanceof Error ? `${error.message} ${String(error.cause ?? '')}` : '';
      if (/daily_run_limit/.test(detail)) throw new WorkspaceError("Daily limit reached: 30 checks.");
      if (/run_cooldown/.test(detail)) throw new WorkspaceError("Wait 10 seconds between checks.");
      if (/task_not_active|task_changed/.test(detail)) throw new WorkspaceError("Task changed. Reload and try again.");
      if (/UNIQUE constraint failed: run.task_id/.test(detail)) throw new WorkspaceError("This task is already running.");
      throw error;
    }
    return id;
  }
  async function claim(id: string) {
    ...
    const claimed = await db.update(run).set({ stage: 1, lease, summary: "Searching" })
      .where(and(eq(run.id, id), eq(run.status, "running"), eq(run.stage, 0))).returning();
    if (!claimed[0]) return null;
    const row = claimed[0];
    const current = await workspace.getTask(row.userId, row.taskId);
    const previous = await db.select().from(finding).where(eq(finding.taskId, row.taskId)).orderBy(desc(finding.date)).limit(200);
    return { run: row, task: current, previous, lease };
  }
  async function progress(id: string, lease: string, stage: number, sources: string[] = []) {
    const changed = await db.update(run).set({ stage, sources }).where(and(eq(run.id, id), eq(run.lease, lease), eq(run.status, "running"))).returning({ id: run.id });
    return changed.length > 0;
  }
  async function complete(id: string, lease: string, result: ResearchResult, now = Date.now()) {
    const date = new Date(now).toISOString();
    const inserts = result.findings.map((item) => raw.prepare(`
    ...
    `).bind(crypto.randomUUID(), item.eventKey, item.version, item.title, item.summary, item.reason, item.url, new URL(item.url).hostname, date, id, lease));
    await raw.batch([
      raw.prepare("UPDATE run SET status = 'completed', stage = 5, finished = ?, summary = ?, sources = ? WHERE id = ? AND lease = ? AND status = 'running'").bind(now, result.summary, JSON.stringify(result.sources), id, lease),
      ...inserts,
      raw.prepare(`UPDATE run SET findings = (SELECT count(*) FROM finding WHERE run_id = ?),
        ...
        WHERE id = ? AND lease = ? AND status = 'completed'`).bind(id, id, id, id, lease),
      raw.prepare(`INSERT OR IGNORE INTO delivery (id, run_id, task_id, target, next_attempt)
  ...
  async function fail(id: string, lease: string | null, summary: string, now = Date.now()) {
    const query = lease === null ? and(eq(run.id, id), eq(run.status, "running")) : and(eq(run.id, id), eq(run.lease, lease), eq(run.status, "running"));
    await db.update(run).set({ status: "failed", outcome: "error", summary, finished: now }).where(query);
  }
  return {
    start, claim, progress, complete, fail,
    async checks(userId: string, now = Date.now()) {
      const row = await raw.prepare("SELECT checks FROM usage WHERE user_id = ? AND day = ?").bind(userId, dayKey("UTC", now)).first<{ checks: number }>();
      return row?.checks ?? 0;
    },
    ...
    async expire(now = Date.now()) {
      await raw.prepare("UPDATE run SET status = 'failed', outcome = 'error', summary = 'Research timed out. Try again.', finished = ? WHERE status = 'running' AND started < ?").bind(now, now - 600_000).run();
    },
  };
}
