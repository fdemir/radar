...
import { publicUrl, researchResultSchema, type Candidate, type ResearchResult } from "@radar/core/research";
import z from "zod";
...
const graphState = new StateSchema({
  queries: z.array(z.string()).default([]), sources: z.array(sourceSchema).default([]),
  result: researchResultSchema.nullable().default(null),
});
const briefSchema = z.object({
  title: z.string().min(1).max(90), brief: z.string().min(1).max(6000), category: categorySchema,
  frequency: z.enum(frequencies), language: languageSchema, reply: z.string().min(1).max(2000),
});

export function createAgent(config: AgentConfig) {
  async function post(url: string, key: string, body: unknown, signal: AbortSignal, service: string) {
  ...
  async function model<T extends z.ZodType>(schema: T, instruction: string, data: unknown, signal: AbortSignal): Promise<z.output<T>> {
    const response = await post(`${config.OPENAI_BASE_URL.replace(/\/$/, "")}/chat/completions`, config.OPENAI_API_KEY, {
    ...
    }, AbortSignal.any([signal, AbortSignal.timeout(60_000)]), "Research assistant");
    const parsed = z.object({ choices: z.array(z.object({ message: z.object({ content: z.string().nullable() }) })).min(1) }).parse(response);
    try { return schema.parse(JSON.parse(parsed.choices[0]!.message.content ?? "")); }
    catch { throw new ResearchError("The assistant returned an incomplete response. Try again."); }
  }
  return {
    async compose(task: TaskInput, message: string) {
      const update = await model(briefSchema,
        ...
        { task, message }, AbortSignal.timeout(65_000));
      const { reply, ...details } = update;
      return { ...task, ...details, messages: [...task.messages, { role: "user" as const, text: message }, { role: "assistant" as const, text: reply }] };
    },
    async research(task: TaskInput, previous: Pick<Candidate, "eventKey" | "version" | "title" | "summary">[], progress: (stage: number, sources?: string[]) => Promise<boolean>): Promise<ResearchResult> {
      ...
      const graph = new StateGraph(graphState)
        .addNode("plan", async () => {
          await step(1);
          return model(z.object({ queries: z.array(z.string().min(1).max(400)).min(1).max(5) }),
            ...
            { brief: task.brief, today: new Date().toISOString().slice(0, 10) }, signal);
        })
        .addNode("search", async (state) => {
          ...
          async function search(queries: string[]) {
            ...
            const response = await fetch(url, { headers: { "X-API-Key": config.TINYFISH_API_KEY }, signal: AbortSignal.any([signal, AbortSignal.timeout(30_000)]), redirect: "manual" });
            if (!response.ok) throw new ResearchError("Web search is unavailable. Try again later.");
            return response.json();
            }));
          }
          const responses = await search(state.queries);
          const hasResults = responses.some((response) => z.object({ results: z.array(z.unknown()) }).parse(response).results.length > 0);
          if (!hasResults && state.queries.length < 5) {
            const refined = await model(z.object({ queries: z.array(z.string().min(1).max(400)).min(1).max(5 - state.queries.length) }),
              ...
              { brief: task.brief, previousQueries: state.queries }, signal);
            await step(1);
            responses.push(...await search(refined.queries));
          }
          const sources = new Map<string, z.infer<typeof sourceSchema>>();
          for (const response of responses) {
            const parsed = z.object({ results: z.array(z.object({ url: z.string(), title: z.string(), content: z.string().optional() })) }).parse(response);
            for (const item of parsed.results) {
              const url = publicUrl(item.url);
              if (url && !sources.has(url)) sources.set(url, { url, title: item.title, content: "" });
            }
          }
        ...
        .addNode("read", async (state) => {
          ...
          const response = await fetch("https://api.fetch.tinyfish.ai", {
            method: "POST", headers: { "X-API-Key": config.TINYFISH_API_KEY, "Content-Type": "application/json" },
            body: JSON.stringify({ urls, purpose: task.brief.slice(0, 2000), format: "markdown", ttl: 0, per_url_timeout_ms: 25000 }),
            signal: AbortSignal.any([signal, AbortSignal.timeout(45_000)]), redirect: "manual",
          });
          if (!response.ok) throw new ResearchError("Source reading is unavailable. Try again later.");
          const parsed = z.object({ results: z.array(z.object({ url: z.string(), final_url: z.string(), text: z.string().nullable() })) }).parse(await response.json());
          const sources = state.sources.flatMap((source) => {
            ...
            return extracted?.text?.trim() && finalUrl ? [{ ...source, url: finalUrl, content: extracted.text.slice(0, 7000) }] : [];
          });
          if (!sources.length) throw new ResearchError("Sources could not be read. Try again later.");
          return { sources };
        })
        .addNode("evaluate", async (state) => {
          await step(3, state.sources.map((source) => source.url));
          if (!state.sources.length) return { result: { summary: task.language === "Türkçe" ? "Yeni eşleşme yok." : "No new matches.", findings: [] } };
          const result = await model(researchResultSchema,
            ...
            { brief: task.brief, today: new Date().toISOString().slice(0, 10), previous: previous.slice(0, 100).map((item) => ({ eventKey: item.eventKey, version: item.version, title: item.title, summary: item.summary.slice(0, 200) })), sources: state.sources }, signal);
          const urls = new Set(state.sources.map((source) => source.url));
          const findings = result.findings.map((item) => {
            const url = publicUrl(item.url);
            if (!url || !urls.has(url)) throw new ResearchError("A finding had an invalid source. Try again.");
            return { ...item, url, eventKey: item.eventKey.trim().toLowerCase(), version: item.version.trim().toLowerCase() };
          });
          await step(4, [...urls]);
          return { result: { ...result, findings } };
        })
        .addEdge(START, "plan").addEdge("plan", "search").addEdge("search", "read")
        .addEdge("read", "evaluate").addEdge("evaluate", END).compile();
