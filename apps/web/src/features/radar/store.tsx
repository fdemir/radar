...
export function WorkspaceProvider({
  ...
  useEffect(() => {
    const timer = setInterval(
      () => {
        ...
        void refresh().catch(() =>
          toast.error("Connection lost. Retrying…", { id: "workspace-connection" }),
        );
      },
      state.runs.some((run) => run.status === "running") ? 2000 : 15000,
    );
    const focus = () => {
      if (!busy.current) void refresh().catch(() => {});
    };
  ...
  async function write(action: () => Promise<unknown>) {
    ...
    try {
      ...
      try {
        await refresh();
        toast.dismiss("workspace-connection");
      } catch {
        toast.error("Saved. Reconnecting…", { id: "workspace-connection" });
      }
      return true;
    } catch (error) {
  ...
  async function save(task: Task) {
    ...
    const ok = await write(async () => {
      ...
      setState((current) => ({
        ...current,
        tasks: [saved, ...current.tasks.filter((item) => item.id !== saved.id)],
      }));
    });
    return ok ? id : null;
  }
  const store: WorkspaceStore = {
    ...
    toggle: async (id) => {
      ...
      if (task && (await save({ ...task, status: task.status === "active" ? "paused" : "active" })))
