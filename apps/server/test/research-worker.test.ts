...
const input: TaskInput = {
  title: "Hono releases", brief: "Find the latest stable Hono release from official notes.",
  category: "Technology", status: "active", frequency: "Daily", time: "09:00",
  language: "English", email: true, messages: [], revision: 0,
};
const source = "https://github.com/honojs/hono/releases/tag/v4.13.7";
const candidate = { title: "Hono 4.13.7", summary: "A stable release with a rendering fix.", reason: "An official stable release.", url: source, eventKey: "hono-release", version: "4.13.7" };
...
beforeAll(async () => {
  const bundled = await build({
    stdin: { contents: `import worker from '../../apps/worker/src/index'; import {createAgent} from '@radar/agent'; export default {...worker, async fetch(req,env) { const {task,message}=await req.json(); return Response.json(await createAgent(env).compose(task,message)); }};`, resolveDir: fileURLToPath(new URL("../", import.meta.url)) },
    bundle: true, write: false, platform: "browser", format: "esm", conditions: ["workerd"], external: ["node:*"],
  });
  runtime = new Miniflare({
    modules: true, script: bundled.outputFiles[0]!.text, compatibilityDate: "2026-07-01", compatibilityFlags: ["nodejs_compat"],
    d1Databases: ["DB"], queueProducers: { RESEARCH_QUEUE: "research" },
    queueConsumers: { research: { maxBatchSize: 1, maxBatchTimeout: 0 } },
    bindings: { OPENAI_API_KEY: "test-model-key", OPENAI_BASE_URL: "https://model.example.com/v1", OPENAI_MODEL: "test-model", TINYFISH_API_KEY: "test-search-key", RESEND_API_KEY: "test-email-key", EMAIL_FROM: "radar@example.com", CORS_ORIGIN: "https://radar.example.com" },
    outboundService: async (request: WorkerRequest) => {
      ...
      calls.push({ host: url.hostname, path: url.pathname, body, key: request.headers.get("Idempotency-Key") });
      const json = (value: unknown, status = 200) => new WorkerResponse(JSON.stringify(value), { status, headers: { "Content-Type": "application/json" } });
      if (url.hostname === "model.example.com") {
        const messages = body.messages as { role: string; content: string }[];
        const data = JSON.parse(messages[1]!.content.slice("Return JSON for this data:\n".length));
        const content = data.message ? { title: input.title, brief: input.brief, category: input.category, frequency: input.frequency, language: input.language, reply: "I will track official stable releases." }
          : data.sources ? { summary: "One stable release.", findings: [{ ...candidate, url: mode === "bad-citation" ? "https://unread.example.com/invented" : source }] }
      ...
      if (url.hostname === "api.search.tinyfish.ai") {
        ...
        return json({ results: mode === "empty" ? [] : [{ url: source + "?utm_source=search", title: "Hono release", snippet: "Stable release" }, { url: "http://127.0.0.1/private", title: "Ignore private URL" }] });
      }
      if (url.hostname === "api.fetch.tinyfish.ai") {
        expect(body.urls).toEqual([source]);
        expect(body.ttl).toBe(0);
        if (mode === "cancel") await d1.prepare("UPDATE task SET status = 'paused' WHERE id = ?").bind(taskId).run();
        return json({ results: [{ url: source, final_url: source, text: "# v4.13.7\nStable Hono release. Fixes rendering in boundary components." }], errors: [] });
      }
      if (url.hostname === "api.resend.com") return json(mode === "mail-error" ? { error: "Try again" } : { id: "sent-1" }, mode === "mail-error" ? 503 : 200);
      throw new Error(`Unexpected outbound host: ${url.hostname}`);
    },
  });
  d1 = await runtime.getD1Database("DB"); worker = await runtime.getWorker();
  ...
  for (const file of (await readdir(folder)).filter((name) => name.endsWith(".sql")).sort()) {
    ...
    await d1.batch(sql.split("--> statement-breakpoint").filter((part) => part.trim()).map((part) => d1.prepare(part)));
  }
  const db = createDb({ DB: d1 }); research = createResearch(db); workspace = createWorkspace(db);
}, 30_000);
afterAll(async () => { await runtime?.dispose(); });
beforeEach(async () => {
  calls = []; mode = "success";
  await d1.prepare("DELETE FROM user").run();
  await d1.prepare("INSERT INTO user (id, name, email, email_verified, created_at, updated_at, username) VALUES ('owner', 'Owner', 'owner@example.com', 1, 0, 0, 'owner')").run();
  taskId = (await workspace.create("owner", input)).id;
});
async function consume(runId: string) {
  const result = await worker.queue("research", [{ id: crypto.randomUUID(), timestamp: new Date(), attempts: 1, body: { runId } }]);
  expect(result.outcome).toBe("ok");
}
const value = (table: string, column: string) => d1.prepare(`SELECT ${column} FROM ${table}`).first(column);
const sent = () => calls.filter((call) => call.host === "api.resend.com");

it("composes tasks using the compatible model inside Workers", async () => {
  const response = await runtime.dispatchFetch("https://radar.example.com/compose", { method: "POST", body: JSON.stringify({ task: input, message: "Only stable releases" }) });
  expect(response.status).toBe(200);
  expect(await response.json()).toMatchObject({ title: input.title, messages: [{ role: "user", text: "Only stable releases" }, { role: "assistant" }] });
});
it("runs TinyFish search and fetch, saves cited findings, and delivers once across queue duplicates", async () => {
  ...
  await consume(id); await consume(id);
