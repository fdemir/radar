...
export type Services = { agent?: ReturnType<typeof createAgent>; enqueue: (runId: string) => Promise<unknown>; emailAvailable: boolean };
...
export function workspaceRoutes(auth: ReturnType<typeof createAuth>, origin: string, db: Database, services?: Services) {
  ...
  routes.get("/workspace", async (c) => c.json({ ...await workspace.snapshot(c.get("userId")), emailAvailable: services?.emailAvailable ?? false, researchAvailable: Boolean(services?.agent) }));
  routes.post("/tasks/compose", json(z.object({ task: taskInputSchema, message: z.string().trim().min(1).max(2000) })), async (c) => {
    if (!services?.agent) return c.json({ error: "Research is not available yet." }, 503);
    const { task, message } = c.req.valid("json");
    if (task.messages.length > 78) return c.json({ error: "Conversation limit reached. Edit the brief directly." }, 400);
    const key = `compose:${c.get("userId")}:${Math.floor(Date.now() / 3_600_000)}`;
    const allowed = await db.$client.prepare(`INSERT INTO rate_limit (id, key, count, last_request) VALUES (?, ?, 1, ?)
      ON CONFLICT(key) DO UPDATE SET count = count + 1 WHERE count < 30 RETURNING count`).bind(crypto.randomUUID(), key, Date.now()).first();
    if (!allowed) return c.json({ error: "Too many setup messages. Try again later." }, 429);
    try { return c.json(await services.agent.compose(task, message)); }
    catch { return c.json({ error: "Unable to update the brief. Try again." }, 502); }
  });
  ...
  routes.patch("/findings/:id", json(z.object({ read: z.boolean().optional(), saved: z.boolean().optional() }).strict()), async (c) => {
    const patch = c.req.valid("json");
    const result = await db.$client.prepare(`UPDATE finding SET read = coalesce(?, read), saved = coalesce(?, saved)
      WHERE id = ? AND task_id IN (SELECT id FROM task WHERE user_id = ?) RETURNING id`)
      .bind(patch.read === undefined ? null : Number(patch.read), patch.saved === undefined ? null : Number(patch.saved), c.req.param("id"), c.get("userId")).first();
    return result ? c.body(null, 204) : c.json({ error: "Finding not found." }, 404);
  });
  routes.post("/findings/read", async (c) => {
    await db.$client.prepare("UPDATE finding SET read = 1 WHERE task_id IN (SELECT id FROM task WHERE user_id = ?)").bind(c.get("userId")).run();
    return c.body(null, 204);
  });
  routes.post("/notices/read", json(z.object({ id: z.string().optional() })), async (c) => {
    await db.$client.prepare("UPDATE delivery SET read = 1 WHERE (? IS NULL OR id = ?) AND task_id IN (SELECT id FROM task WHERE user_id = ?)")
      .bind(c.req.valid("json").id ?? null, c.req.valid("json").id ?? null, c.get("userId")).run();
    return c.body(null, 204);
  });
  return routes;
}
