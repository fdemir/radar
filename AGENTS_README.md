...
Research uses a bounded tool loop. The model sees search titles and snippets, chooses 2–3 relevant pages to read, and can make 1–2 targeted follow-up searches when promising leads need clarification. It can follow links returned by a page reader. A run allows at most seven model turns, five searches, five distinct page reads, and five findings. Repeated provider requests are skipped, and the final model turn must summarize the available evidence. Search filters follow the brief; a daily schedule does not restrict an open listing to items posted today.

Search snippets guide discovery but are not accepted as verified findings. Model output must cite a page the run actually read and include a short verbatim source excerpt. Quotes are checked against the page text before saving; unsupported quotes are omitted. Missing optional details do not disqualify a match. Relevant sources with no new events do not trigger extra research. Incomplete research is labeled separately from no new matches. Finding cards and details offer an expandable Source excerpt; older findings remain readable without one. Each run has a three-minute research deadline; the scheduler recovers stuck work after ten minutes. Three consecutive failures pause the task. Tool decisions and individual completed calls are checkpointed, so a rate-limited batch resumes at the pending call without repeating its successful requests.
